...
fn get_linter_list() -> Vec<&'static Linter> {
    vec![
        ...
        &Linter {
            description: "Check that no symbol from fujicoin-build-config.h is used without the header being included",
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./cmake/fujicoin-build-config.h.in";
    ...
    let print_affected_files = |mode: bool| {
        ...
        let defines_files = check_output(
            git()
                ...
                .args(get_pathspecs_exclude_subtrees())
                .args([
                    // These are exceptions which don't use fujicoin-build-config.h, rather CMakeLists.txt adds
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <fujicoin-build-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
One or more files use a symbol declared in the fujicoin-build-config.h header. However, they are not
...
even though fujicoin-build-config.h indicates that a faster feature is available and should be used.
...
#include <fujicoin-build-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
None of the files use a symbol declared in the fujicoin-build-config.h header. However, they are including
