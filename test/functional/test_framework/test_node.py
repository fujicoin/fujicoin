...
# 'fujicoin' command, it may internally insert more args, which must be
...
FUJICOIN_PID_FILENAME_DEFAULT = "fujicoind.pid"
...
class TestNode():
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then fujicoind would bind P2P ports on
        ...
        # fujicoin.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
    ...
    def wait_for_rpc_connection(self, *, wait_for_import=True):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            try:
                ...
                # Suppress if cookie file isn't generated yet and no rpcuser or rpcpassword; fujicoind may be starting.
                if "No RPC credentials" not in str(e):
                    raise
                latest_error = suppress_error("missing_credentials", e)
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error(f"Unable to connect to fujicoind after {self.rpc_timeout}s (ignored errors: {dict(suppressed_errors)!s}{'' if latest_error is None else f', latest: {latest_error[0]!r}/{latest_error[1]}'})")
