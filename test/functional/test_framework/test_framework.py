#!/usr/bin/env python3
# Copyright (c) 2014-present The Fujicoin Core developers
...
class Binaries:
    """Helper class to provide information about fujicoin binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke fujicoind"
        return self._argv("node", self.paths.fujicoind, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke fujicoin-cli"
        # Add -nonamed because "fujicoin rpc" enables -named by default, but fujicoin-cli doesn't
        return self._argv("rpc", self.paths.fujicoincli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke fujicoin-tx"
        return self._argv("tx", self.paths.fujicointx)

    def util_argv(self):
        "Return argv array that should be used to invoke fujicoin-util"
        return self._argv("util", self.paths.fujicoinutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke fujicoin-wallet"
        return self._argv("wallet", self.paths.fujicoinwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke fujicoin-chainstate"
        return self._argv("chainstate", self.paths.fujicoinchainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command. It
        either uses the fujicoin wrapper executable (if FUJICOIN_CMD is set or
        need_ipc is True), or the direct binary path (fujicoind, etc). When
        bin_dir is set (by tests calling binaries from previous releases) it
        always uses the direct path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.fujicoin_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the fujicoin
            # wrapper binary and append -m so it calls multiprocess binaries.
            fujicoin_cmd = self.paths.fujicoin_cmd or [self.paths.fujicoin_bin]
            return fujicoin_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]
...
class FujicoinTestFramework(metaclass=FujicoinTestMetaClass):
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            "fujicoin": "FUJICOIN_BIN",
            "fujicoind": "FUJICOIND",
            "fujicoin-cli": "FUJICOINCLI",
            "fujicoin-util": "FUJICOINUTIL",
            "fujicoin-tx": "FUJICOINTX",
            "fujicoin-chainstate": "FUJICOINCHAINSTATE",
            "fujicoin-wallet": "FUJICOINWALLET",
        }
        # Set paths to fujicoin core binaries allowing overrides with environment
        # variables.
        ...
        # FUJICOIN_CMD environment variable can be specified to invoke fujicoin
        # wrapper binary instead of other executables.
        paths.fujicoin_cmd = shlex.split(os.getenv("FUJICOIN_CMD", "")) or None
        return paths
    ...
    def skip_if_no_fujicoin_tx(self):
        """Skip the running test if fujicoin-tx has not been compiled."""
        if not self.is_fujicoin_tx_compiled():
            raise SkipTest("fujicoin-tx has not been compiled")
    ...
    def skip_if_no_fujicoin_chainstate(self):
        """Skip the running test if fujicoin-chainstate has not been compiled."""
        if not self.is_fujicoin_chainstate_compiled():
            raise SkipTest("fujicoin-chainstate has not been compiled")
    ...
    def is_fujicoin_tx_compiled(self):
        """Checks whether fujicoin-tx was compiled."""
        return self.config["components"].getboolean("BUILD_FUJICOIN_TX")
    ...
    def is_fujicoin_chainstate_compiled(self):
        """Checks whether fujicoin-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_FUJICOIN_CHAINSTATE")
