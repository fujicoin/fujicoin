...
class FilelockTest(FujicoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second fujicoind instance using the same blocksdir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second fujicoind using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second fujicoind instance shutdown
        pid_file = datadir / FUJICOIN_PID_FILENAME_DEFAULT
