...
We can import the `TestShell` by adding the path of the configured Fujicoin Core
...
```
>>> import sys
>>> sys.path.insert(0, "/path/to/fujicoin/build/test/functional")
...
* `/path/to/fujicoin/build/test/functional/combine_logs.py
...
| `cachedir` | `"/path/to/fujicoin/build/test/cache"` | Sets the fujicoind datadir directory. |
| `chain`  | `"regtest"` | Sets the chain-type for the underlying test fujicoind processes. |
| `configfile` | `"/path/to/fujicoin/build/test/config.ini"` | Sets the location of the test framework config file. |
