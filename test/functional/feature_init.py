...
from test_framework.test_node import (
    FUJICOIN_PID_FILENAME_DEFAULT,
    ErrorMatch,
)
from test_framework.util import assert_equal


class InitTest(FujicoinTestFramework):
    """
    ...
    def init_stress_test(self):
        ...
        def sigterm_node():
            if platform.system() == 'Windows':
                ...
                # fujicoind to perform any shutdown logic.
    ...
    def init_pid_test(self):
        FUJICOIN_PID_FILENAME_CUSTOM = "my_fancy_fujicoin_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = FUJICOIN_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / FUJICOIN_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / FUJICOIN_PID_FILENAME_CUSTOM
