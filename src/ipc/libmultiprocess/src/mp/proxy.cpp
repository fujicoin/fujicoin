...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking fujicoin code so they have to run
    ...
    // functions run, so client objects are fully disconnected before fujicoin
    // code in the destructors are run. This way if the fujicoin code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In fujicoin core, running these callbacks in the right order is
