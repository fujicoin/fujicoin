...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The fujicoin-cli utility provides a command line interface to interact with a " CLIENT_NAME " RPC server.\n"
                ...
                "\n"
                "Usage: fujicoin-cli [options] <command> [params]\n"
                "or:    fujicoin-cli [options] -named <command> [name=value]...\n"
                "or:    fujicoin-cli [options] help\n"
                "or:    fujicoin-cli [options] help <command>\n"
                "\n";
...
public:
        ...
        if (!args.empty()) {
            ...
            if (args.size() > 1) {
                if (std::string_view s{args.at(1)}; n && (s == "o" || s == "outonly")) {
                    ...
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nFor more information, run: fujicoin-cli -netinfo help", s));
                } else {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nThe outonly argument is only valid for a level greater than 0 (the first argument). For more information, run: fujicoin-cli -netinfo help", s));
                }
            }
        }
        UniValue result(UniValue::VARR);
    ...
    const std::string m_help_doc{
        ...
        + strprintf("> fujicoin-cli -netinfo %d outonly\n\n", NETINFO_MAX_LEVEL) +
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcwallet = {})
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\n"
                    "Make sure the fujicoind server is running and that you are connecting to the correct RPC port.\n"
                    "Use \"fujicoin-cli -help\" for more info.",
...
static void ParseError(const UniValue& error, std::string& strPrint, int& nRet)
{
    if (error.isObject()) {
        ...
        if (err_code.isNum() && err_code.getInt<int>() == RPC_WALLET_NOT_SPECIFIED) {
            ...
            strPrint += " (run \"fujicoin-cli -h\" for help or \"fujicoin-cli listwallets\" to see which wallets are currently loaded).";
        }
    } else {
