This directory contains the source code for the Fujicoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Fujicoin UI.
...
- ClientModel is used by the main application `fujicoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a fujicoin:... link from a browser).
...
* UI elements like FujicoinAmountField, which inherit from QWidget.
* `fujicoinstrings.cpp`: automatically generated
* `fujicoinunits.(h/cpp)`: FJC / mFJC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/fujicoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `fujicoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `fujicoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `fujicoin-qt`
9. You're all set! Start developing, building, and debugging the Fujicoin Core GUI
