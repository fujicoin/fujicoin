...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Fujicoin
    //   new: C:\Users\Username\AppData\Local\Fujicoin
    // macOS: ~/Library/Application Support/Fujicoin
    // Unix-like: ~/.fujicoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Fujicoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Fujicoin";
#else
    fs::path pathRet;
