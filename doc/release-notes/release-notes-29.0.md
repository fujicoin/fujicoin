...
previously Fujicoin Core would listen for incoming Tor connections on `127.0.0.1:8334`.
...
127.0.0.1:5556`, or configure fujicoind with `-bind=127.0.0.1:8334=onion` to get the previous behavior.
(#31223)
...
- As a safety check, Fujicoin core will **fail to start** when `-blockreservedweight` init parameter value is lower than `2000` weight units. Fujicoin Core will also **fail to start** if the `-blockmaxweight` or `-blockreservedweight` init parameter exceeds consensus limit of `4,000,000 WU`.
...
```
cmake -B build
cmake --build build --target fujicoind
cmake --install build --component fujicoind
```
...
- A new tool [`utxo_to_sqlite.py`](https://github.com/fujicoin/fujicoin/blob/v29.0/contrib/utxo-tools/utxo_to_sqlite.py)
