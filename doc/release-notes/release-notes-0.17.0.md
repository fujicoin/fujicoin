...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `fujicoin.conf` file, not inside included files or from
  ...
  as fujicoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the fujicoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=fujicoin
    test.uacomment=fujicoin-testnet
...
be used in V0.17 by starting fujicoind with the '-deprecatedrpc=accounts'
...
Overall, the construction of a fully signed Fujicoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Fujicoin transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in FJC. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When fujicoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If fujicoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart fujicoind with
...
- When running fujicoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If fujicoind
...
- #11962 `0a01843` add seed.fujicoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in FJC (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI fujicoin:// (krab)
...
- #13528 `b877c39` Move FujicoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libfujicoin-util to libfujicoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for fujicoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling fujicoin-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use FujicoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to fujicoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use FujicoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use FujicoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use FujicoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for FujicoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_fujicoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.fujicoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_fujicoin` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in fujicoind systemd service file (Flowdalic)
- #12652 `55f490a` fujicoin-cli: Provide a better error message when fujicoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to fujicoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for fujicoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` fujicoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix fujicoin-cli --version (Empact)
...
- #12877 `2b54155` Use fujicoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update fujicoin-dot-org links in release-process.md (fanquake)
...
- #13905 `73a09b4` fixed fujicoin-cli -help output for help2man (hebasto)
