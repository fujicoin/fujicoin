...
The `settings.json` file contains dynamic settings that are set by the Fujicoin Core GUI and RPCs at runtime, and augment or replace the static settings specified in the `fujicoin.conf` file.

Command line options also augment or replace `fujicoin.conf` options, and can be useful for scripting and debugging.

It is possible to see which setting values are in use by checking `debug.log` output. Any unrecognized options that are found in `fujicoin.conf` also show up as warnings in `debug.log` output.
