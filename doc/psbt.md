...
PSBT is an interchange format for Fujicoin transactions that are not fully signed
...
#### Multisig with multiple Fujicoin Core instances
...
Fujicoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `fujicoin-cli` is used.
...
Later, when *V* FJC has been received on *Amulti*, and Bob and Carol want to
