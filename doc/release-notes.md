...
Fujicoin Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. Fujicoin
...
New `fujicoin` command
---------------------

- A new `fujicoin` command line tool has been added to make features more discoverable
  and convenient to use. The `fujicoin` tool just calls other executables and does not
  implement any functionality on its own. Specifically `fujicoin node` is a synonym for
  `fujicoind`, `fujicoin gui` is a synonym for `fujicoin-qt`, and `fujicoin rpc` is a synonym
  for `fujicoin-cli -named`. Other commands and options can be listed with `fujicoin help`.
  The new `fujicoin` command is an alternative to calling other commands directly, but it
...
- The new `fujicoin` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `fujicoin -m node -ipcbind=unix` it will
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of Fujicoin Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in Fujicoin Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the fujicoin-wallet tool,
