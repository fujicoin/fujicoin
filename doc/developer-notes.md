...
```shell
$ valgrind --suppressions=contrib/valgrind.supp build/bin/test_fujicoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/bin/test_fujicoin --log_level=test_suite
$ valgrind -v --leak-check=full build/bin/fujicoind -printtoconsole
...
# A coverage report will now be accessible at `./build/test_fujicoin.coverage/index.html`,
...
```shell
llvm-cov show \
    --object=build/bin/test_fujicoin \
    --object=build/bin/fujicoind \
    ...
    --project-title="Fujicoin Core Coverage Report"
```
...
llvm-cov show \
    ...
    --show-expansions \
    --output-dir=build/coverage_report \
    --project-title="Fujicoin Core Fuzz Coverage Report"
```
...
Normally, these are maintained by active developers of Fujicoin Core, in which case
...
The tool instructions also include a list of the subtrees managed by Fujicoin Core.
...
- src/leveldb
  - Upstream at https://github.com/google/leveldb ; maintained by Google. Open
    important PRs to the Fujicoin Core subtree to avoid delay.
