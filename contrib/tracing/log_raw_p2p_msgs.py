...
# eBPF Limitations:
#
# Fujicoin P2P messages can be larger than 32kb (e.g. tx, block, ...). The eBPF
...
def main(pid):
    print(f"Hooking into fujicoind with pid {pid}")
    fujicoind_with_usdts = USDT(pid=int(pid))
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("USAGE:", sys.argv[0], "<pid of fujicoind>")
        exit()
    pid = sys.argv[1]
    main(pid)
