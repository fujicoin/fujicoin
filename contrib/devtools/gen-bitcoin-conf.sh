...
EXAMPLE_CONF_FILE=${EXAMPLE_CONF_FILE:-$SHARE_EXAMPLES_DIR/fujicoin.conf}

[ ! -x "$FUJICOIND" ] && echo "$FUJICOIND not found or not executable." && exit 1

DIRTY=""
VERSION_OUTPUT=$($FUJICOIND --version)
if [[ $VERSION_OUTPUT == *"dirty"* ]]; then
  DIRTY="${DIRTY}${FUJICOIND}\n"
fi

if [ -n "$DIRTY" ]
then
  echo -e "WARNING: $FUJICOIND was built from a dirty tree.\n"
  echo -e "To safely generate a fujicoin.conf file, please commit your changes to $FUJICOIND, rebuild, then run this script again.\n"
fi

echo 'Generating example fujicoin.conf file in share/examples/'
...
## Generated by contrib/devtools/gen-fujicoin-conf.sh.
##
...
# parse the output from fujicoind --help
