...
be used in Fujicoin Core with the `-asmap` option.
...
to binary before being used with Fujicoin Core.
...
the `fujicoin-cli getnodeaddresses` command.
...
```
fujicoin-cli getnodeaddresses 0 > addrs.json
```
